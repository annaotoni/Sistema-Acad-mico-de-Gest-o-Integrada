...
const makeService = () => new LiveClassesService(mockRepo as any, mockNotifications as any);

const professor = { sub: 'prof-1', role: Role.PROFESSOR, tenantId: 'tenant-1', jti: '', iat: 0, exp: 0 };
...
describe('getUpcoming', () => {
  it('proíbe acesso de não-aluno', async () => {
    await expect(makeService().getUpcoming(professor)).rejects.toBeInstanceOf(ForbiddenException);
  });
...
describe('update', () => {
  it('lança NotFoundException se não existe', async () => {
    ...
    await expect(makeService().update('lc-x', {}, professor)).rejects.toBeInstanceOf(NotFoundException);
  });

  it('rejeita transição inválida AGENDADA → ENCERRADA', async () => {
    mockRepo.findById.mockResolvedValue(agendadaClass);
    await expect(
      makeService().update('lc-1', { status: LiveClassStatus.ENCERRADA }, professor),
    ).rejects.toBeInstanceOf(BadRequestException);
  });

  it('permite AGENDADA → AO_VIVO e notifica', async () => {
    mockRepo.findById.mockResolvedValue(agendadaClass);
    mockRepo.update.mockResolvedValue({ ...agendadaClass, status: LiveClassStatus.AO_VIVO });
    mockNotifications.publish.mockResolvedValue(undefined);

    await makeService().update('lc-1', { status: LiveClassStatus.AO_VIVO }, professor);
    expect(mockNotifications.publish).toHaveBeenCalled();
    expect(mockRepo.update).toHaveBeenCalledWith('lc-1', { status: LiveClassStatus.AO_VIVO });
  });

  it('AO_VIVO → ENCERRADA com gravação vincula material', async () => {
    mockRepo.findById.mockResolvedValue({ ...agendadaClass, status: LiveClassStatus.AO_VIVO });
    mockRepo.createRecordingMaterial.mockResolvedValue({ id: 'mat-1' });
    mockRepo.setRecordingMaterial.mockResolvedValue({});
    mockRepo.update.mockResolvedValue({});
    mockNotifications.publish.mockResolvedValue(undefined);

    await makeService().update('lc-1', { status: LiveClassStatus.ENCERRADA, recordingUrl: 'https://rec.url' }, professor);
