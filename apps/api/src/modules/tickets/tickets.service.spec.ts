...
const makeService = () => new TicketsService(mockRepo as any, mockNotifications as any);
...
describe('getTicket', () => {
  it('aluno não acessa ticket de outro aluno', async () => {
    mockRepo.findById.mockResolvedValue({ ...abertoTicket, studentId: 'outro' });
    await expect(makeService().getTicket('tkt-1', aluno)).rejects.toBeInstanceOf(ForbiddenException);
  });

  it('lança NotFoundException quando não existe', async () => {
    mockRepo.findById.mockResolvedValue(null);
    await expect(makeService().getTicket('tkt-x', secretaria)).rejects.toBeInstanceOf(NotFoundException);
  });
});

describe('addMessage', () => {
  it('bloqueia mensagem em ticket encerrado', async () => {
    mockRepo.findById.mockResolvedValue({ ...abertoTicket, status: TicketStatus.RESOLVIDO });
    await expect(makeService().addMessage('tkt-1', { content: 'oi' }, aluno))
  ...
  it('secretaria responde: avança para EM_ATENDIMENTO e notifica aluno', async () => {
    ...
    await makeService().addMessage('tkt-1', { content: 'Resposta' }, secretaria);
    expect(mockRepo.updateStatus).toHaveBeenCalledWith('tkt-1', TicketStatus.EM_ATENDIMENTO);
    expect(mockNotifications.publish).toHaveBeenCalled();
  });

  it('aluno responde: não avança status nem notifica', async () => {
    mockRepo.findById.mockResolvedValue({ ...abertoTicket, status: TicketStatus.EM_ATENDIMENTO });
...
describe('updateStatus', () => {
  it('rejeita atualização em ticket já encerrado', async () => {
    mockRepo.findById.mockResolvedValue({ ...abertoTicket, status: TicketStatus.FECHADO });
    await expect(makeService().updateStatus('tkt-1', { status: TicketStatus.RESOLVIDO }, secretaria))
      .rejects.toBeInstanceOf(BadRequestException);
  });
});
